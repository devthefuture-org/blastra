...
async function initializeGit(targetDir) {
  try {
    ...
    console.warn(
      chalk.yellow(
        "⚠️ Git initialization failed. You can manually initialize your space-time coordinates later."
      )
    )
    return false
  }
...
async function initializeHusky(targetDir) {
  try {
    ...
    console.warn(
      chalk.yellow("⚠️ Husky initialization failed. You can set up git hooks manually later.")
    )
    return false
  }
}
